...
class RenderTests(unittest.TestCase):
    ...
    def render(self, err_exclude):
        ...
        site = {
            "name": "e.oreol-2010.ru",
            "domain": "e.oreol-2010.ru",
            "backend_ip": "10.0.0.1",
            "backend_port": 5010,
            "err_limit": 250,
        }
    ...
    def test_a_site_without_exclusions_declares_none(self):
        self.assertIn("set-var(txn.site_e_oreol_2010_ru_excl) bool(false)", self.baseline)
        self.assertNotIn("site_e_oreol_2010_ru_ex1_path", self.baseline)

    def test_an_exact_path_rule_is_rendered(self):
        rendered = self.render([{"path": "/", "methods": ["GET"]}])
        self.assertIn("acl site_e_oreol_2010_ru_ex1_path path -i /", rendered)
        self.assertIn("acl site_e_oreol_2010_ru_ex1_meth method GET", rendered)
    ...
    def test_a_prefix_rule_uses_path_beg(self):
        ...
        self.assertIn(
            "acl site_e_oreol_2010_ru_ex1_path path_beg -i /webapi/entry.cgi",
            rendered,
        )
    ...
    def test_a_regular_expression_rule_uses_path_reg(self):
        ...
        self.assertIn(
            "acl site_e_oreol_2010_ru_ex1_path path_reg -i ^/static/", rendered
        )
    ...
    def test_the_variable_is_actually_set(self):
        ...
        rendered = self.render([{"path_beg": "/webapi/entry.cgi"}])
        self.assertIn(
            "set-var(txn.site_e_oreol_2010_ru_excl) bool(true)", rendered
        )
    ...
    def test_the_rule_is_scoped_to_its_own_site(self):
        ...
        line = [
            ...
            if "site_e_oreol_2010_ru_excl) bool(true)" in row
        ][0]
        self.assertIn("host_e_oreol_2010_ru", line)

    def test_methods_are_optional(self):
        rendered = self.render([{"path_beg": "/webapi/"}])
        self.assertNotIn("site_e_oreol_2010_ru_ex1_meth", rendered)
        line = [
            row for row in rendered.splitlines()
            if "site_e_oreol_2010_ru_excl) bool(true)" in row
        ][0]
    ...
    def test_several_rules_get_distinct_names(self):
        ...
        self.assertIn("site_e_oreol_2010_ru_ex1_path", rendered)
        self.assertIn("site_e_oreol_2010_ru_ex2_path", rendered)

    def test_an_empty_rule_renders_nothing(self):
        # A row left blank on the page must not produce an acl matching all.
        rendered = self.render([{"path": "", "methods": ["GET"]}, {}])
        self.assertNotIn("site_e_oreol_2010_ru_ex1_path", rendered)
        self.assertNotIn("site_e_oreol_2010_ru_ex2_path", rendered)

    def test_the_exclusion_still_gates_the_tracking_rule(self):
        rendered = self.render([{"path_beg": "/webapi/entry.cgi"}])
        tracking = [
            row for row in rendered.splitlines()
            if "track-sc1 src table tbl_err_e_oreol_2010_ru" in row
        ]
        self.assertTrue(tracking)
        self.assertIn("site_e_oreol_2010_ru_excl", tracking[0])
